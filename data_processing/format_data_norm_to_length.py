...
class format_data(object):
	...
	#refdict2=pickle.load(open('../Results/access_EC_name_map.p','rb'))
	
	#refdict2_r=dict()
	#for r in refdict2:
	#	refdict2_r[refdict2[r]]=r
	...
	for t in T:
		...
		for i in outdata:
			#print i
			if i in TL:
				outdata[i]['t'+str(t)][k]=outdata[i]['t'+str(t)][k]/(TL[i]*norm)			
				#print TL[i]*norm
			else:
				outdata[i]['t'+str(t)][k]=0.0
