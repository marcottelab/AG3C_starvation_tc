...
def run(prefix,datatype):
	...
	#Normalize counts based on read depth
	...
	for t in T:
		...
		if t==336:
			av_counts['t'+str(t)]=counts.filter(regex='336').mean(axis=1)
			st_counts['t'+str(t)]=counts.filter(regex='336').mean(axis=1)
	...
	for i in counts.index:
		maximum=av_counts[av_counts.index==i].filter(regex='t').max(axis=1)[0]
		minimum=av_counts[av_counts.index==i].filter(regex='t').min(axis=1)[0]
		#print np.array(maximum)/np.array(minimum)
